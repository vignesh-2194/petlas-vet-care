...
export const AdminDashboard: React.FC = () => {
    const [activeTab, setActiveTab] = useState<"appointments" | "products" | "gallery">("appointments");
    ...
    const handleAddProductSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_API_URL || 'https://petlas-vet-care.onrender.com'}/api/products`, productPayload);
    ...
    const handleDeleteProduct = async (productId: string) => {
        ...
        try {
            await axios.delete(`${import.meta.env.VITE_API_URL || 'https://petlas-vet-care.onrender.com'}/api/products/${productId}`);
    ...
    const handleUpdateProductSubmit = async (productId: string) => {
        ...
        try {
            await axios.put(`${import.meta.env.VITE_API_URL || 'https://petlas-vet-care.onrender.com'}/api/products/${productId}`, updatedPayload);
    ...
    const handleUpdateStatus = async (appointmentId: string, newStatus: string) => {
        try {
            await axios.patch(`${import.meta.env.VITE_API_URL || 'https://petlas-vet-care.onrender.com'}/api/appointments/${appointmentId}`, { status: newStatus });
            ...
            <div className="flex-1 p-8 overflow-y-auto">
                {loading ? (
                    ...
                    <section className="space-y-6 text-left">
                        <h2 className="text-xl font-extrabold text-slate-800 tracking-tight">Gallery Administration Board</h2>
                        <p className="text-xs text-slate-500">Remove unwanted clinic memory nodes directly from MongoDB Atlas</p>
                        <Gallery />
                    </section>
                                        ...
                                        <div key={productItemId} className="p-4 border border-slate-100 rounded-xl bg-slate-50/50 shadow-sm hover:shadow-md transition-all relative text-left">
                                            {isEditing ? (
                                                <div className="space-y-3 text-xs">
                                                    ...
                                                    <select value={editProdAvailability} onChange={(e) => setEditProdAvailability(e.target.value)} className="w-full p-2 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 focus:outline-none">
